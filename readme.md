...
^[Invoke-WebRequest -Uri https://wazuh.com/resources/blog/emulation-of-attack-techniques-and-detection-with-wazuh/sysmonconfig.xml -OutFile sysmonconfig.xml]({"attribution":{"attributableIndex":"0-2"}})
.\S^[ysmon64.exe -accepteula -i sysmonconfig.xml]({"attribution":{"attributableIndex":"0-3"}})
...
Voici un plan clair et des commandes concrètes pour simuler des actions de ransomware sur ta machine Windows 11, afin de vérifier que Wazuh/Wazoo détecte correctement les comportements suspects :
...
# Relance l’agent Wazuh
^[Restart-Service -Name wazuh]({"attribution":{"attributableIndex":"0-4"}})

Ce setup permet à Wazuh de remonter les événements Sysmon, y compris création de fichiers, processus lancés, commandes système, etc.  ￼ ￼
...
Génère alertes prévues dans les règles Wazuh ￼
...
Déclenche des alertes FIM sur création de fichier suspect  ￼
...
Ce sont des cadences classiques de ransomware (BlackSuit, Phobos…) ￼
...
➡ Monte un vrai comportement d’ajout + suppression FIM détectable.
...
# Place RanSim.ps1 sur ta machine
.\R^[anSim.ps1 -Mode encrypt -TargetPath C:]({"attribution":{"attributableIndex":"2154-4"}})\Users\Public\TestFolder

Cela chiffre tous les fichiers .txt, .docx, etc. dans la cible, sans note de rançon  ￼
